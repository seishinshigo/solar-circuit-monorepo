...
- **ワークオーダーID**: WO-20250709-014
- **タイトル**: Add CI step to verify commit scope

- **関連ドキュメント**: projects/solar_circuit/collaboration/design_docs/009_workorder.md
...
- [ ] Update `report-autosave.yml` to include `Verify commit scope` step
- [ ] Implement script to fail CI when unintended files are staged
...
- [ ] CI fails if `git diff` shows non-WO files staged
- [ ] CI log shows clarity on staged file list
...
{
  "id": "WO-20250709-014",
  "title": "Add CI step to verify commit scope",
